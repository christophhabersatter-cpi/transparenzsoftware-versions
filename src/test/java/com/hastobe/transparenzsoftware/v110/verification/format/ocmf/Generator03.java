...
import com.hastobe.transparenzsoftware.v110.verification.xml.OffsetDateTimeAdapter;
...
public class Generator03 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        PrintWriter pkDataOut = new PrintWriter(String.format("test_files/data/%d_key_data.txt", ts_beginning));
...
//        System.out.println("OCMF|");
        ...
        int max = 40000;
        for (int i = 0; i < max; i++) {
            String fileName = String.format("test_files/data/%d_data_%06d.xml", ts_beginning, i + 1);
            System.out.println(String.format("Create data set %d from %d (File %s)", i + 1, max, fileName));

            double valueStart = Math.random() * 10000;
            double valueStop = valueStart + Math.random() * 1000;
            ...
            String rdDataStart = prepareSdData(offsetDateTime, 'S', valueStart, 'B', 1523, 'G');
            String rdDataStop = prepareSdData(offsetDateTime, 'S', valueStop, 'E', 1523, 'G');
            ...
            String sdData = String.format("{\"FV\":\"1.0\",\"VI\":\"HTB\",\"VV\":\"0.0.1\",\"PG\":\"T12345\",\"MV\":\"HTB\",\"MM\":\"HTB\",\"MS\":\"HTBGenerated1\",\"MF\":\"1.0\",\"IS\":true,\"IL\":\"VERIFIED\",\"IF\":[\"RFID_PLAIN\",\"OCPP_RS_TLS\"],\"IT\":\"ISO14443\",\"ID\":\"%s\",\"RD\":[%s]}", idTag, rdData);
            byte[] signature = sign(privateKeyParameters, ecSpec, sdData);
            String ocmfData = String.format("OCMF|%s|{\"SA\":\"ECDSA-secp256k1-SHA256\",\"SD\":\"%s\"}", sdData, Utils.bytesToHex(signature));
    ...
    private static String prepareSdData(OffsetDateTime dateTime, char synchronicity, double value, char tx, int eventIndex, char st) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss,SSSZ");
        String rdDataTemplate = String.format("{\"TM\":\"%s %s\",\"TX\":\"%s\",\"RV\":%.4f,\"RI\":\"1-b:1.8.e\",\"RU\":\"kWh\",\"EI\":%s,\"ST\":\"%s\"}",
