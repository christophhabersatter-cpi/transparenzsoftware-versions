...
public class OCMFVerificationParser implements VerificationParser {
    ...
    public OCMF parseString(String data) throws OCMFValidationException {
        ...
        if (version < 0.5) {
            ocmfPayloadData = gson.fromJson(rawData, com.hastobe.transparenzsoftware.v100.verification.format.ocmf.v02.OCMFPayloadData.class);
        } else if (version <= 1.0) {
            ocmfPayloadData = gson.fromJson(rawData, com.hastobe.transparenzsoftware.v100.verification.format.ocmf.v05.OCMFPayloadData.class);
        } else {
