...
/**
 * Class keep book of signature data fetched by sml
 * until byte 33 everything is only compliant to edl 40 basic.
 * <p>
...
public abstract class SMLSignature {
    ...
    public final static int RESERVED_BYTES_OFFSET = 173;
    public final static int RESERVED_BYTES_LENGTH = 147;
    ...
    /**
     * Stated in spec as Geräteeinzelidentifikation (<=> Server-ID)
     * 10 bytes long
     */
     ...
     * <p>
     * - first byte is the lsb of the seconds index
     * - last byte ist the msb of the seconds index
     */
    private byte[] secondsIndex;

    /**
     * Pagination
     * - first byte is lsb of pagination
     * - last byte msb of pagination
     */
    private byte[] pagination;

    /**
     * Stated in spec as OBIS-Kennzahl
     * 5 bytes long
     */
    private byte[] obisNr;
    ...
    /**
     * Zählerstand
     * 8 bytes ( 8byte unsigned integer)
     */
    private byte[] meterPosition;

    /**
     * Byte at pos 31 -> high byte of logbook entry index
     * Byte at pos 32 -> low byte of logbook entry index
     */
    private byte[] bytesLog;

    /**
     * customer id (Kunden-ID)
     * 128 bytes long if LSB at the beginning the end has to be
     * filled up with 0x00
     */
    ...
    /**
     * Takes the status as int will only have a look at the
     * LSB
     *
    ...
    /**
     * Will set the unit byte with an integer.
     * We only care about the LSB.
     *
    ...
    /**
     * Sets the meter position switching the order of the byte array
     * so that the LSB becomes the MSB
     *
    ...
    /**
     * Sets the meter position with long value which will be transferred
     * to a byte array
     *
     * @param meterPosition meterPosition as long
     * @throws SMLValidationException if meter position cannot be transferred to
     *                                a 23 bytes long array
     */
     ...
     * @param highByte MSB logbook byte
     *                 to a 2 bytes long array
     */
     ...
     *
     * @param contractId customer id in bytes 128 bytes long
     * @param fillUp     controls if bytes will be filled up starting at the beginning
     * @throws SMLValidationException if customer id is not 128 bytes long
     */
    ...
    /**
     * Sets the seconds index value with a int value will be transformed
     * to byte array
    ...
    /**
     * Creates the array ready for the hashing and signature creation
     * ehz SML Kundenidentifikation 2 page 6 and 7
     * <p>
     * Takes the basic signature data and starts adding data
     * on SIGNATURE_OFFSET
     *
     * @return byte array
     */
    public byte[] buildExtendedSignatureData() {
        ...
        System.arraycopy(getTimestamp(), 0, signature, TIMESTAMP_OFFSET, TIMESTAMP_LENGTH);
        signature[STATUS_OFFSET] = status;
        System.arraycopy(pagination, 0, signature, PAGINATION_INDEX_OFFSET, PAGINATION_INDEX_LENGTH);
        System.arraycopy(secondsIndex, 0, signature, SECONDS_INDEX_OFFSET, SECONDS_INDEX_LENGTH);
        System.arraycopy(obisNr, 0, signature, OBIS_ID_OFFSET, OBIS_ID_LENGTH);
        signature[UNIT_OFFSET] = unit;
        signature[SCALER_OFFSET] = (byte) (scaler & 0xFF);

        System.arraycopy(meterPosition, 0, signature, METER_POSITION_OFFSET, METER_POSITION_LENGTH);
        System.arraycopy(bytesLog, 0, signature, LOGBUCH_OFFSET, LOGBUCH_LENGTH);
        ...
        System.arraycopy(getTimestampContractId(), 0, signature, CONTRACT_ID_TIMESTAMP_OFFSET, CONTRACT_ID_TIMESTAMP_LENGTH);
        // fill up empty bytes at the end is done by java itself
        // arrays are initialised wit the default value which is 0 on bytes
        // https://docs.oracle.com/javase/tutorial/java/nutsandbolts/datatypes.html
        return signature;
    ...
    /**
     * Takes the time in as an int will do the
    ...
    protected static byte[] timeToBytes(OffsetDateTime dateTime) {
        int seconds = (int) (dateTime.toEpochSecond() + dateTime.getOffset().getTotalSeconds());
